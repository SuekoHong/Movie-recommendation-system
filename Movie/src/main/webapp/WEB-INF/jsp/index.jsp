...
--%>
<%@ page import="com.movies.dao.Movie" %>
...
<head>
    ...
    <link href="${pageContext.request.contextPath}/assets/css/init.css" rel="stylesheet">
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/__init__.css" />
    <!-- 电影推荐模块CSS-->
...
<nav class="navbar navbar-default" role="navigation" style="background-color: black;margin-bottom: 0%">
    ...
    <!-- 判断用户是否登录-->
    <!-- 从session域中获取user对象，若为空，则未登录-->
    <c:if test="${sessionScope.user == null}">

        <a  class="dream" href="${ pageContext.request.contextPath }/page/register" id="register" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-user"></span> 注册</a>
        <a  class="dream" href="${ pageContext.request.contextPath }/page/login" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-log-in"></span> 登录</a>
        <a  class="dream" href="${ pageContext.request.contextPath }/page/profile" id="register" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span style="color: white" class="glyphicon glyphicon-facetime-video"></span> 想看</a>
        <a  class="dream" href="${ pageContext.request.contextPath }/page/profile" id="register" style="float: right;color: white;font-size: 13pt;margin-top: 10px;margin-right: 10px"><span class="glyphicon glyphicon-facetime-video"></span> 最近看过</a>

    </c:if>
    <!-- 从session域中获取user对象，若不为空，则登录-->
    <c:if test="${sessionScope.user != null}">
        <!-- 手机号即为账号-->
        ...
        <a class="dream" onclick='javascript:$.post("/page/profile",{"id":"${sessionScope.user.userTel}"}, function (data) {
                if (data=="success") {
                    ...
                    <div id="list" class="list">
                        ...
                        <c:if test="${sessionScope.movie != null}">
                            <c:forEach var="item"   items="${sessionScope.movie}">
                                <a class="item"   target="_blank" name="imgitem" id="${item.id}" onclick='javascript:$.post("/Customer/Description",{id:$(this).attr("id")}, function (data) {
                                    if (data == "success") {
                                    ...
                                    <div class="cover-wp">
                                        ...
                                        <!--海报链接不是404-->
                                        <c:if test="${item.moviePoster!='http://image.tmdb.org/t/p/w185'}">
                                            <img src="${item.moviePoster}" alt="${item.moviePoster}" data-x="1500" data-y="2200" style="width: 100%"/>
                                        </c:if>
                                    </div>
                                    <p>${item.movieName}
                ...
                if (data.status == 200) {
                    if(data.data.length!=0) {
                        $.each(data.data, function (i, item) {
                            ...
                            if (item.moviePoster == "http://image.tmdb.org/t/p/w185"||item.moviePoster==null)
                                headHtml = headHtml.replace(/{cover}/g, "https://img3.doubanio.com/view/photo/s_ratio_poster/public/p2512283982.jpg");
                            else
                                headHtml = headHtml.replace(/{cover}/g,item.moviePoster);
